...
## 25. Надёжность автоанализа Key и обложек Telegram-бота (2026-08-13)

Проверен путь `upload → asset.process → audio.analyze → DB → catalog/UI` на
треке `What The Fuzz (Extended Mix)` (`019ffaee-4602-755b-b3ad-34c680a84e26`).
В момент исходного снимка анализ ещё был в состоянии `PENDING`: BPM уже был
доступен, а Key — нет. После завершения анализа продовая запись корректна:
`bpm=129`, `camelotKey=7B`, `analysisStatus=DONE`; Studio, `/pool/track/...`
и `/api/artwork/<versionId>` загружались без ошибок.

- **Camelot boundary:** `src/lib/camelot.ts` получил
  `normalizeCamelotKey(value: unknown)`. Все helpers безопасно принимают
  отсутствующие и повреждённые данные; невалидный Key становится `null`, а UI
  отображает `—`, не вызывая строковые методы на `null`/`undefined`.
- **Анализатор:** `essentia-analyzer.ts` валидирует типы ответа WASM для BPM,
  tonic и scale до сохранения результата. Неопределённая тональность не мешает
  сохранить BPM и завершить частичный анализ.
- **DB → UI:** `catalog.repository.ts` нормализует `camelotKey` при построении
  DTO, а Studio использует нормализованное значение в select. Повреждённая
  историческая запись не способна сломать клиентский рендер или фильтрацию.
- **Telegram artwork:** `/api/bot/upload` больше не сохраняет JPEG/WebP как
  `artwork.png` с ложным `image/png`: формат, расширение и MIME сохраняются
  согласованно. Финальные аудио и обложка записываются до постановки
  `asset.process`, поэтому внешняя очередь не может извлечь/перезаписать
  устаревшую обложку. Разрешены JPEG, PNG и WebP.
- **Artwork route:** ошибка чтения из storage логируется и возвращает
  предсказуемый `502 artwork_unavailable`, а не необработанное исключение.

**Проверка:** `npx tsc --noEmit`, `npm run lint` (один существующий warning в
`scripts/backfill-artwork.mjs`), `npm run build`; ручная проверка production
Studio, публичной страницы и изображения обложки `600×600`.
